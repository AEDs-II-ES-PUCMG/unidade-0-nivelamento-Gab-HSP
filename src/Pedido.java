...
public class Pedido {
	...
	public Pedido(LocalDate dataPedido, int formaDePagamento) {
		...
		itensDePedido = new ItemDePedido[MAX_PRODUTOS];
		quantProdutos = 0;
	...
	public boolean incluirProduto(ItemDePedido novo) {
		...
		if (quantProdutos < MAX_PRODUTOS) {
			itensDePedido[quantProdutos++] = novo;
			return true;
		}
		return false;
	}
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantProdutos; i++) {
			valorPedido += itensDePedido[i].getProduto().valorVenda();
		}
     ...
     * Contém um cabeçalho com sua data e o número de itensDePedido no.getProduto() pedido.
	...
	public String toString() {
		...
		for (int i = 0; i < quantProdutos; i++ ) {
			stringPedido.append(itensDePedido[i].getProduto().toString() + "\n");
		}
	...
	public void mesclarPedido(ItemDePedido[] outroPedido) {

    for (int j = 0; j < outroPedido.length; j++) {

        if (outroPedido[j] == null) {
            continue;
        }

        boolean encontrado = false;

        // verifica se já existe no pedido atual
        for (int i = 0; i < quantProdutos; i++) {

            if (itensDePedido[i].getProduto().equals(outroPedido[j].getProduto())) {

                int novaQuantidade = itensDePedido[i].getQuantidade() + outroPedido[j].getQuantidade();
                itensDePedido[i].setQuantidade(novaQuantidade);
                encontrado = true;
                break;
            }
        }
        ...
        if (!encontrado) {
            ...
            if (quantProdutos >= MAX_PRODUTOS) {
                throw new IllegalStateException("Não é possível inserir. Quantidade máxima de produtos excedida");
            }

            itensDePedido[quantProdutos++] = outroPedido[j];
        }
    }
}
}